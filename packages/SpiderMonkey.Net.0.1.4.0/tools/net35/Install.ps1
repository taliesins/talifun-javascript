...
elseif($project.ExtenderNames -contains "WebApplication") {
	...
	if($depAsm) {
		...
		if($amd64) {
			...
			Add-ProjectItem $amd64 (Join-Path $amd64dir "nspr4.dll");
			Add-ProjectItem $amd64 (Join-Path $amd64dir "smnetjs.dll");
		}
		$x86 = Ensure-Folder $depAsm "x86";
		if($x86) {
			$x86dir = (Join-Path $nativeBinDirectory "x86")
			
			Add-ProjectItem $x86 (Join-Path $x86dir "nspr4.dll");
			Add-ProjectItem $x86 (Join-Path $x86dir "smnetjs.dll");
		}
	}
}
else {
